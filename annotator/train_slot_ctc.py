...
class VizCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for i in range(self.num_display_words):
            ...
            pylab.xlabel('Truth = \'%s\'\nDecoded = \'%s\'' % ('|'.join([labels[x] for x in word_batch['the_labels'][i] if x < len(labels)]), res[i]))
        fig = pylab.gcf()
...
if __name__ == '__main__':
    ...
    if not os.path.exists(final_output_json):
        ...
        if not os.path.exists(current_model_path):
            ...
            inner = Bidirectional(CuDNNGRU(rnn_size, return_sequences=True))(inner)
            inner = Bidirectional(CuDNNGRU(rnn_size, return_sequences=True))(inner)
            ...
            labels_input = Input(name='the_labels', shape=[9], dtype='float32')
