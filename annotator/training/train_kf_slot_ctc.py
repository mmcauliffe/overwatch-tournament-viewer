...
class DataGenerator(object):
    ...
    def generate_class_weights(self):
        ...
        for i, (next_ind, path) in enumerate(self.data_indices.items()):
            with h5py.File(path, 'r') as hf5:
                ...
                y_train = hf5['train_label_sequence']
        ...
        label_weights = create_class_weight(c)
    ...
    def _data_generation(self, i_s, i_e, train=True):
        ...
        if orig_i_e > next_ind:
            ...
            next_file = False
        with h5py.File(path, 'r') as hf5:
            input['the_input'] = hf5["{}_img".format(pre)][i_s:i_e, ...]
            # if self.subtract_mean:
...
def check_val_errors(model, gen):
    ...
    for n, i in enumerate(batches_list):
        ...
        skip = True
        for i in range(X['the_labels'].shape[0]):
            actual = labels_to_text(X['the_labels'][i])
            if 'rip-tire_npc' in actual:
                skip = False
        res = decode_batch(model.predict_on_batch, X['the_input'])
        if skip:
            continue
        for i in range(len(res)):
            actual = labels_to_text(X['the_labels'][i])
            if 'rip-tire_npc' not in actual:
                continue
            #print(X['round'][i], X['time_point'][i])
            print('actual', actual)
            print('predicted', res[i])
            if gen.subtract_mean:
                cv2.imshow('frame', np.swapaxes(X['the_input'][i] + gen.mm[0, ...], 0, 1))
            else:
...
if __name__ == '__main__':
    ...
    label_weights = gen.generate_class_weights()
    print(label_weights)
