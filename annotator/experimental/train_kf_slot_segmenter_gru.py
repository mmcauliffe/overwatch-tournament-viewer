...
working_dir = r'E:\Data\Overwatch\models\kf_cnn_slot_gru'
os.makedirs(working_dir, exist_ok=True)

train_dir = r'E:\Data\Overwatch\training_data\kf_slot_gru'
...
class DataGenerator(object):
    ...
    def _data_generation(self, i_s, i_e, train=True):
        ...
        #cv2.imshow('frame', np.swapaxes(input['main_input'][0, 0, ...], 0, 1))
        #cv2.waitKey(0)
        input['spectator_mode_input'] = np.zeros((i_e-i_s, 100, 250, spectator_mode_count))
        m = sparsify(self.hdf5_file["{}_spectator_mode_label".format(pre)][i_s:i_e, 0], spectator_mode_count)
        ...
        #    cv2.waitKey(0)
        output = {}
        output['labels_output'] = sparsify_3d(self.hdf5_file["{}_label".format(pre)][i_s:i_e], class_count)
        ...
        for k, v in class_counts.items():
            ...
            output[output_name] = sparsify_2d(self.hdf5_file["{}_{}_label".format(pre, k)][i_s:i_e], v)
        if debug:
...
def check_val_errors(model, gen):
    ...
    for n, i in enumerate(batches_list):
        ...
        actual_inds = y['labels_output'].argmax(axis=3)
        print(y['labels_output'][0,0, ...])
        print(actual_inds.shape)
        for t_ind in range(X['main_input'].shape[0]):
            for j_ind in range(X['main_input'].shape[1]):
                cnn_inds = preds[0][t_ind,j_ind].argmax(axis=1)
                predicted_labels = convert_output(cnn_inds)
                print(actual_inds[t_ind,j_ind, :])
                actual_labels = convert_output(actual_inds[t_ind,j_ind, :])
                box = X['main_input'][t_ind,j_ind, ...]
                print('spec_mode', spectator_modes[X['spectator_mode_input'][t_ind,0,0, :].argmax(axis=0)])
                ...
                for output_ind,(k, s) in enumerate(sets.items()):
                    print(k)
                    cnn_ind = preds[output_ind+ 1][t_ind,j_ind].argmax(axis=-1)
                    actual_ind = y['{}_output'.format(k)][t_ind, j_ind].argmax(axis=-1)
                    print('predicted', s[cnn_ind])
                    print('actual', s[actual_ind])
...
def create_1d_model(input_shape):
    ...
    x = TimeDistributed(Bidirectional(CuDNNGRU(128, return_sequences=True)))(x)
    x = TimeDistributed(Dropout(0.5))(x)
    x = TimeDistributed(Bidirectional(CuDNNGRU(128, return_sequences=True)))(x)
    #seq_x = TimeDistributed(Bidirectional(CuDNNGRU(128)))(x)

    #seq_x = Bidirectional(CuDNNGRU(128, return_sequences=True))(seq_x)
    #seq_x = Dropout(0.5)(seq_x)
    #seq_x = Bidirectional(CuDNNGRU(128, return_sequences=True))(seq_x)

    outputs = []
    output_name = 'labels_output'
    outputs.append(TimeDistributed(TimeDistributed(Dense(class_count, activation='softmax')), name=output_name)(x))
    #for k, count in class_counts.items():
    #    output_name = k + '_output'
    #    outputs.append(TimeDistributed(Dense(count, activation='softmax'), name=output_name)(seq_x))

    model = Model(inputs=[main_input, spectator_mode_input], outputs=outputs)
    return model

def create_2d_model(input_shape):
    img_w = input_shape[-3]
    img_h = input_shape[-2]
    pool_size = 2
    conv_filters = 32
    ...
    act ='relu'
    main_input = Input(shape=input_shape, name='main_input')
    x = TimeDistributed(Conv2D(conv_filters, kernel_size=kernel_size,
                                               activation='relu'))(main_input)
    x = TimeDistributed(Conv2D(conv_filters, kernel_size=kernel_size,
                                               activation='relu'))(x)
    x = TimeDistributed(MaxPooling2D(pool_size=(pool_size, pool_size)))(x)
    x = TimeDistributed(Dropout(0.25))(x)

    x = TimeDistributed(Conv2D(conv_filters, kernel_size, activation='relu'))(x)
    x = TimeDistributed(Conv2D(conv_filters, kernel_size, activation='relu'))(x)
    x = TimeDistributed(MaxPooling2D(pool_size=(pool_size, pool_size)))(x)
    x = TimeDistributed(Dropout(0.25))(x)

    conv_to_rnn_dims = (img_w // (pool_size ** 2), (img_h // (pool_size ** 2)) * conv_filters)
    x = TimeDistributed(Reshape(target_shape=conv_to_rnn_dims, name='reshape'))(x)

    # cuts down input size going into RNN:
    x = TimeDistributed(Dense(time_dense_size, activation=act, name='dense1'))(x)
...
if __name__ == '__main__':
    ...
    from keras.layers import Conv2D, MaxPooling2D, Flatten, Dropout, Dense, Input, LSTM, TimeDistributed, Bidirectional, CuDNNGRU, CuDNNLSTM, Conv1D, MaxPooling1D, Reshape
    from keras.regularizers import l1_l2

    input_shape = (100, 250, 32, 3)
    params = {'dim_x': 32,
              'dim_y': 210,
              'dim_z': 3,
              'batch_size': 4,
    ...
    if not os.path.exists(final_output_json):
        ...
        if not os.path.exists(current_model_path):
            model = create_2d_model(input_shape)
            model.summary()
